...
static ur_error_t sid_allocated_handler(message_info_t *info, uint8_t *tlvs,
  ...
  if (allocated_sid == NULL || allocated_node_type == NULL || netinfo == NULL ||
      mcast == NULL) {
...
void umesh_mm_init_tv_base(mm_tv_t *tv, uint8_t type) { tv->type = type; }
...
ur_error_t handle_address_error(message_t *message) {
  ...
  if (memcmp(info->src_mac.addr.addr,
             g_mm_state.attach_context.attach_node->mac, EXT_ADDR_SIZE) != 0) {
    return error;
  }
...
uint16_t umesh_mm_get_meshnetsize(void) { return nd_get_meshnetsize(NULL); }
...
uint8_t *umesh_mm_get_local_uuid(void) { return g_mm_state.device.uuid; }
...
node_mode_t umesh_mm_get_mode(void) { return g_mm_state.device.mode; }
...
int8_t umesh_mm_get_seclevel(void) { return g_mm_state.device.seclevel; }
...
uint16_t umesh_mm_get_channel(hal_context_t *hal) { return hal->channel; }
...
node_state_t umesh_mm_get_device_state(void) { return g_mm_state.device.state; }
...
uint8_t umesh_mm_get_leader_mode(void) { return g_mm_state.leader_mode; }
...
uint8_t umesh_mm_get_reboot_flag(void) { return g_mm_state.device.reboot_flag; }
