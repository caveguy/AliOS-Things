...
static ur_error_t update_link_cost(link_nbr_stats_t *stats)
{
    ...
    if (stats->link_accept) {
        new = ((uint32_t)LINK_ESTIMATE_COEF * stats->link_request) /
              stats->link_accept;
    } else {
        new = LINK_COST_MAX;
    }
    ...
    if (stats->link_cost == 0xffff) {
        ...
        old = stats->link_cost;
        stats->link_cost =
          ((uint32_t)((LINK_ESTIMATE_COEF - LINK_ESTIMATE_UPDATE_ALPHA) * old) +
           ((uint32_t)(LINK_ESTIMATE_UPDATE_ALPHA * new))) /
          LINK_ESTIMATE_COEF;
...
static ur_error_t send_link_request(ur_addr_t *dest, uint8_t *tlvs,
                                    uint8_t tlvs_length)
{
...
#ifdef CONFIG_AOS_MESH_LOWPOWER
    uint8_t tlv_types[4] = { TYPE_UCAST_CHANNEL, TYPE_TIMESTAMP, TYPE_TIME_SLOT,
                             TYPE_BUFQUEUE_SIZE };
    uint8_t tlv_length =
      (umesh_get_mode() & MODE_RX_ON) ? 2 : sizeof(tlv_types);
#else
...
static void update_neighbors_link_cost(hal_context_t *hal)
{
    ...
    slist_for_each_entry_safe(&hal->neighbors_list, list_tmp, nbr, neighbor_t,
                              next)
    {
        error = update_link_cost(&nbr->stats);
        if (error != UR_ERROR_NONE ||
            nbr->stats.reverse_rssi < RSSI_THRESHOLD ||
            (umesh_now_ms() - nbr->last_heard) >
              (hal->neighbor_alive_interval / 2)) {
...
static void handle_link_quality_timer(void *args)
{
    ...
    ur_start_timer(&hal->link_quality_update_timer,
                   hal->link_quality_update_interval, handle_link_quality_timer,
                   hal);
                                ...
                                uint8_t *tlvs, uint16_t length, bool is_attach)
{
    ...
    slist_for_each_entry(&hal->neighbors_list, nbr, neighbor_t, next)
    {
        ...
        MESH_LOG_INFO("sid %04x mac " EXT_ADDR_FMT " is replaced", nbr->sid,
                      EXT_ADDR_DATA(nbr->mac));
        goto get_nbr;
    }
    return NULL;
...
static bool neighbor_is_alive(hal_context_t *hal, neighbor_t *nbr)
{
    ...
    if ((umesh_get_mode() & MODE_RX_ON) == 0 || (nbr->mode & MODE_RX_ON) == 0) {
        threshold = SCHEDULE_SLOT_INTERVAL * SCHEDULE_SLOTS_SIZE *
                    LINK_ESTIMATE_SENT_THRESHOLD;
    } else
#endif
    {
...
static void handle_update_nbr_timer(void *args)
{
    ...
    slist_for_each_entry_safe(&hal->neighbors_list, list_tmp, node, neighbor_t,
                              next)
    {
        ...
        MESH_LOG_INFO("%04x neighbor " EXT_ADDR_FMT " become inactive", sid,
                      EXT_ADDR_DATA(node->mac));
                            ...
                            uint16_t length, bool is_attach)
{
    ...
    ssid_info =
      (mm_ssid_info_tv_t *)umesh_mm_get_tv(tlvs, length, TYPE_SSID_INFO);
    channel =
      (mm_channel_tv_t *)umesh_mm_get_tv(tlvs, length, TYPE_UCAST_CHANNEL);
    ...
    if (network->router->sid_type == STRUCTURED_SID &&
        ...
        if (sid_node == NULL) {
            nbr->state = STATE_NEIGHBOR;
        } else if (is_direct_child(network->sid_base,
                                   info->src.addr.short_addr)) {
...
neighbor_t *get_neighbor_by_mac_addr(const uint8_t *addr, hal_context_t **hal)
{
    ...
    {
        slist_for_each_entry(&hal_context->neighbors_list, nbr, neighbor_t,
                             next)
        {
                                ...
                                hal_context_t **hal)
{
    ...
    slist_for_each_entry(hals, hal_context, hal_context_t, next)
    {
        slist_for_each_entry(&hal_context->neighbors_list, nbr, neighbor_t,
                             next)
        {
            if (nbr->sid == sid && nbr->netid == meshnetid &&
                nbr->state > STATE_INVALID) {
                if (hal) {
                    *hal = hal_context;
                }
...
static ur_error_t send_link_accept_and_request(ur_addr_t *dest, uint8_t *tlvs,
                                               uint8_t tlvs_length)
{
    ur_error_t         error = UR_ERROR_MEM;
    message_t *        message;
    ...
    message =
      mf_build_message(MESH_FRAME_TYPE_CMD, COMMAND_LINK_ACCEPT_AND_REQUEST,
...
static ur_error_t send_link_accept(ur_addr_t *dest, uint8_t *tlvs,
                                   uint8_t tlvs_length)
{
    ur_error_t         error = UR_ERROR_MEM;
    message_t *        message;
                               ...
                               int16_t hdr_ies_limit)
{
    ...
    nbr =
      get_neighbor_by_sid(info->dest.netid, info->dest.addr.short_addr, NULL);
    if (nbr) {
        rssi =
          (mm_rssi_tv_t *)(hal->frame.data + info->header_ies_offset + offset);
        umesh_mm_init_tv_base((mm_tv_t *)rssi, TYPE_REVERSE_RSSI);
        if ((umesh_get_mode() & MODE_RX_ON) &&
            (nbr->flags & NBR_LINK_ESTIMATED) == 0) {
            if (nbr->stats.reverse_rssi > RSSI_THRESHOLD) {
                nbr->flags |= NBR_LINK_ESTIMATED;
            }
            rssi->rssi = 127;
    ...
    while (tv.type != TYPE_HEADER_IES_TERMINATOR) {
        switch (tv.type) {
            case TYPE_REVERSE_RSSI:
                message_copy_to(message, offset, (uint8_t *)&rssi,
                                sizeof(rssi));
                if (rssi.rssi == 127) {
                    send_link_accept(&info->src_mac, tlv_types,
                                     sizeof(tlv_types));
                }
                ...
                break;
            case TYPE_MODE:
                message_copy_to(message, offset, (uint8_t *)&mode,
                                sizeof(mode));
                info->mode = mode.mode;
...
ur_error_t handle_link_accept_and_request(message_t *message)
{
    ...
    message_copy_to(message, sizeof(mm_header_t), tlvs, tlvs_length);
    channel =
      (mm_channel_tv_t *)umesh_mm_get_tv(tlvs, tlvs_length, TYPE_UCAST_CHANNEL);
    if (channel) {
