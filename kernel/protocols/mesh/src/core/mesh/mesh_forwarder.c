...
static inline bool is_local_ucast_address(message_info_t *info)
{
    ...
    switch (addr->len) {
        case SHORT_ADDR_SIZE:
            ...
            break;
        case EXT_ADDR_SIZE:
            if (memcmp(addr->addr, umesh_mm_get_local_uuid(),
                       sizeof(addr->addr)) == 0) {
                matched = true;
            }
            break;
                                       ...
                                       message_info_t *info, uint8_t *buf)
{
    ...
    info->type =
      (control->control[0] & MESH_FRAME_TYPE_MASK) >> MESH_FRAME_TYPE_OFFSET;
    info->hops =
      (control->control[0] & MESH_HOPS_LEFT_MASK) >> MESH_HOPS_LEFT_OFFSET;
    ...
    switch ((control->control[1] & MESH_HEADER_DEST_MASK) >>
        ...
        case EXT_ADDR_MODE:
            ...
            memcpy(info->dest.addr.addr, ext_addr->addr,
                   sizeof(ext_addr->addr));
    ...
    switch ((control->control[1] & MESH_HEADER_DEST2_MASK) >>
        ...
        case EXT_ADDR_MODE:
            ...
            memcpy(info->dest2.addr.addr, ext_addr->addr,
                   sizeof(ext_addr->addr));
            offset += sizeof(mesh_ext_addr_t);
            break;
        case BCAST_ADDR_MODE:
                                  ...
                                  message_info_t *   info)
{
    ...
    if (info->dest.netid == BCAST_NETID) {
        control->control[1] |=
          (BCAST_DEST_NETID << MESH_HEADER_DESTNETID_OFFSET);
    ...
    switch (info->dest2.addr.len) {
        ...
        case SHORT_ADDR_SIZE:
            control->control[1] |=
              (SHORT_ADDR_MODE << MESH_HEADER_DEST2_OFFSET);
            short_addr       = (mesh_short_addr_t *)(hal->frame.data + length);
        ...
        case EXT_ADDR_SIZE:
            ...
            ext_addr = (mesh_ext_addr_t *)(hal->frame.data + length);
            memcpy(ext_addr->addr, info->dest2.addr.addr,
                   sizeof(ext_addr->addr));
            length += sizeof(mesh_ext_addr_t);
            break;
        default:
...
static ur_error_t send_fragment(network_context_t *network, message_t *message)
{
...
#ifdef CONFIG_AOS_MESH_LOWPOWER
    if (next_node && (next_node->mode & MODE_RX_ON) == 0 &&
        (next_node->flags & NBR_WAKEUP) == 0) {
    ...
    if (info->flags & INSERT_MESH_HEADER) {
        ...
        message_copy_to(message, message->frag_offset, hal->frame.data,
                        header_length);
    ...
    if (message->frag_offset == 0) {
        ...
        if (frag_length > (mtu - header_length)) {
            ...
            *((uint16_t *)&frag_header) = htons(*((uint16_t *)&frag_header));
            ...
            frag_length =
              (mtu + 1 - sizeof(frag_header_t) - header_length) & 0xff8;
    ...
    if (append_length > 0) {
        memcpy(hal->frame.data + header_length, (uint8_t *)&frag_header,
               append_length);
    }
    ...
    if ((info->flags & ENCRYPT_ENABLE_FLAG) &&
        umesh_aes_encrypt(
          key, KEY_SIZE, hal->frame.data + info->header_ies_offset,
    ...
    if (next_node) {
        ...
        error = hal_umesh_send_ucast_request(hal->module, &hal->frame, &mac,
                                             handle_sent, hal);
    } else {
...
static neighbor_t *get_next_node(message_info_t *info)
{
    ...
    if (same_net == true) {
        if (same_subnet == true) {
            next_hop =
              ur_router_get_next_hop(network, info->dest.addr.short_addr);
            ...
            next_hop =
              ur_router_get_next_hop(network, get_leader_sid(info->dest.netid));
            next = get_neighbor_by_sid(get_main_netid(info->dest.netid),
                                       next_hop, NULL);
        }
    } else {
        next = umesh_mm_get_attach_candidate();
...
static void get_tx_network_context(message_info_t *info)
{
    ...
    if (network == NULL) {
        if (info->dest.addr.len == EXT_ADDR_SIZE) {
            ...
            nbr = get_neighbor_by_sid(info->dest.netid,
                                      info->dest.addr.short_addr, &hal);
        }
        if (nbr) {
...
static void set_src_info(message_info_t *info)
{
    ...
    set_mesh_short_addr(&info->src, umesh_mm_get_meshnetid(info->network),
...
static bool get_rx_network_context(message_info_t *info, hal_context_t *hal)
{
    ...
    if (is_unique_netid(info->dest.netid)) {
        info->network =
          get_network_context_by_meshnetid(info->dest.netid, false);
        if (info->network == NULL) {
            info->network = (is_subnet(info->dest.netid)
                               ? get_sub_network_context(hal)
                               : get_hal_default_network_context(hal));
        }
    } else if (is_unique_netid(info->src.netid)) {
        info->network =
          get_network_context_by_meshnetid(info->src.netid, false);
    }
    ...
    if (is_unique_netid(info->src.netid) &&
        !is_unique_netid(info->dest.netid) && is_diff_level) {
        return false;
    }
#else
    info->network = get_default_network_context();
    ...
    if (info->type == MESH_FRAME_TYPE_DATA) {
        ...
        if (info->flags & INSERT_MCAST_FLAG) {
            ...
            slist_for_each_entry(hals, hal, hal_context_t, next)
            {
                ...
                mcast_message =
                  message_alloc(message_get_msglen(message) + append_length,
                                MESH_FORWARDER_1);
                ...
                mcast_message->info->network =
                  (void *)get_hal_default_network_context(hal);
                message_queue_enqueue(&hal->send_queue[DATA_QUEUE],
                                      mcast_message);
...
static bool proxy_check(message_t *message)
{
    ...
    if (is_bcast_sid(&info->src) ||
        is_same_mainnet(info->src.netid, umesh_mm_get_meshnetid(network)) ==
          false ||
        ...
        if (is_unique_netid(info->dest.netid) &&
            (network == NULL ||
             info->dest.addr.short_addr != umesh_mm_get_local_sid())) {
            return false;
        }
...
static void handle_received_frame(void *context, frame_t *frame,
                                  frame_info_t *frame_info, int error)
{
    message_t *       message;
    ...
    if (umesh_error == UR_ERROR_NONE && (info.flags & ENCRYPT_ENABLE_FLAG)) {
        info.key_index =
          (umesh_mm_get_attach_state() == ATTACH_REQUEST ? ONE_TIME_KEY_INDEX
                                                         : GROUP_KEY1_INDEX);
        key = get_key(info.key_index);
        umesh_error =
          umesh_aes_decrypt(key, KEY_SIZE, frame->data + info.header_ies_offset,
...
static void send_datagram(void *args)
{
    ...
    if (error == UR_ERROR_NONE) {
        ur_start_timer(&hal->sending_timer, SENDING_TIMEOUT,
...
static void       handle_datagram(void *args)
{
    ...
    if (info->type == MESH_FRAME_TYPE_DATA) {
        ...
        if (is_mcast_header(*nexth)) {
            ...
            {
                relay_message =
                  message_alloc(message_get_msglen(message), MESH_FORWARDER_3);
                if (relay_message != NULL) {
                    message_copy(relay_message, message);
                    relay_message->info->network =
                      (void *)get_hal_default_network_context(hal);
                    message_queue_enqueue(&hal->send_queue[DATA_QUEUE],
                                          relay_message);
                    umesh_task_schedule_call(send_datagram, hal);
...
const ur_link_stats_t *mf_get_stats(hal_context_t *hal)
{
    ...
    for (index = CMD_QUEUE; index <= PENDING_QUEUE; index++) {
        hal->link_stats.send_queue_size +=
          message_queue_get_size(&hal->send_queue[index]);
    }
...
{
    MESH_LOG_DEBUG("mesh forwarder mesh interface down handler, reason %d",
                   state);
