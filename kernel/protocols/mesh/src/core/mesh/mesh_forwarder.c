...
static inline bool is_local_ucast_address(message_info_t *info) {
  ...
  case SHORT_ADDR_SIZE:
    ...
    break;
  case EXT_ADDR_SIZE:
    if (memcmp(addr->addr, umesh_mm_get_local_uuid(), sizeof(addr->addr)) ==
        0) {
      matched = true;
    }
    break;
...
void message_sent_task(void *args) {
  ...
  if (hal->frag_info.offset >= msg_length && hal->last_sent == UR_ERROR_NONE) {
    free_msg = true;
...
static ur_error_t send_fragment(network_context_t *network,
                                message_t *message) {
...
static neighbor_t *get_next_node(message_info_t *info) {
  ...
  if (same_net == true) {
    if (same_subnet == true) {
      ...
      next =
          get_neighbor_by_sid(get_main_netid(info->dest.netid), next_hop, NULL);
    }
  } else {
    next = umesh_mm_get_attach_candidate();
  }
...
static bool get_rx_network_context(message_info_t *info, hal_context_t *hal) {
  ...
  if (is_unique_netid(info->dest.netid)) {
    ...
    if (info->network == NULL) {
      info->network =
          (is_subnet(info->dest.netid) ? get_sub_network_context(hal)
  ...
  if (is_unique_netid(info->src.netid) && !is_unique_netid(info->dest.netid) &&
      is_diff_level) {
    return false;
  }
#else
  ...
  if (info->type == MESH_FRAME_TYPE_DATA) {
    ...
    if (info->flags & INSERT_MCAST_FLAG) {
      ...
      slist_for_each_entry(hals, hal, hal_context_t, next) {
        ...
        mcast_message = message_alloc(
            message_get_msglen(message) + append_length, MESH_FORWARDER_1);
...
static void send_datagram(void *args) {
  ...
  if (error == UR_ERROR_NONE) {
    ur_start_timer(&hal->sending_timer, SENDING_TIMEOUT, handle_sending_timer,
                   hal);
  } else {
    hal->last_sent = error;
    umesh_task_schedule_call(message_sent_task, hal);
