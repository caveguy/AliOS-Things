...
#include <pthread.h>
static int dda_task_new(const char *name, void (*fn)(void *), void *arg,
                        int stacksize) {
  pthread_t tid;
...
static void dda_task_exit(int code) { pthread_exit(NULL); }

extern int aos_task_new(const char *name, void (*fn)(void *), void *arg,
                        int stacksize);
...
static int create_socket(void) {
  ...
  LOGI(MYTAG, "agent started @ %s %d", inet_ntoa(my_addr.sin_addr),
       agent_info.port);
  return ret;
out1:
  close(sockfd);
...
static void check_sending_queue(void *d1, void *d2) {
  ...
  do {
    sending_ent_t *ent =
        dlist_first_entry(&agent->sending_queue, sending_ent_t, next);
    ipc_msg_t *cmsg = (ipc_msg_t *)ent->buf;
    struct sockaddr_in *paddrto = &agent->master_addr;

    sendto(agent->sockfd, ent->buf, ent->size, 0, (struct sockaddr *)paddrto,
           sizeof(*paddrto));
...
static int send_master(agent_info_t *agent, void *buf, int size) {
  ...
  if (dlist_empty(&agent->sending_queue)) {
    sendto(agent->sockfd, buf, size, 0, (struct sockaddr *)paddrto,
           sizeof(*paddrto));
    return size;
  }
...
static int send_data_ext(agent_info_t *agent, int type, int cmd,
                         uint64_t cmd_priv, int dst_id, const void *buf,
                         int len) {
...
static int send_data(agent_info_t *agent, int type, int cmd, int dst_id,
                     void *buf, int len) {
...
static int connect_master(agent_info_t *agent) {
  ...
  return send_data(agent, TYPE_MGMT, CMD_NEED_ACK(CMD_MGMT_DEVICE_UP),
                   MASTER_ID, &req, sizeof req);
}
...
static void handle_data(agent_info_t *agent, ipc_msg_t *msg,
...
static void handle_mesh(agent_info_t *agent, ipc_msg_t *msg,
                        struct sockaddr *paddr) {
...
static void handle_cmd_response(char *buf, int len, void *priv) {
  ...
  if (!buf && !len) {
    send_data_ext(&agent_info, TYPE_P2P, CMD_P2P_RES, res->cmd_private,
                  res->cmd_src, res->buf, strlen(res->buf) + 1);
    cpu_event_free(priv);
    return;
  }
...
static void got_p2p_request(const void *arg) {
  compound_msg_t *cmsg = (compound_msg_t *)arg;
  struct cmd_res *res =
      (struct cmd_res *)cpu_event_malloc(sizeof(*res) + CLI_RES_SIZE);
  res->buf[0] = 0;
...
static void report_agent_status(void *d1, void *d2) {
  ...
  send_data(agent, TYPE_MGMT, CMD_MGMT_STATUS, MASTER_ID, &status,
            sizeof status);

  eloop_register_timeout(5, 0, report_agent_status, 0, agent);
}

static void handle_event(agent_info_t *agent, ipc_msg_t *msg,
                         struct sockaddr *paddr) {
...
static void handle_message(agent_info_t *agent, ipc_msg_t *msg,
                           struct sockaddr *paddr) {
  uint16_t cmd = msg->cmd;
...
void dda_log(char *buf, ...) {
  ...
  va_end(args);
  send_data(&agent_info, TYPE_MISC, CMD_MISC_LOG, 0, buffer,
            strlen(buffer) + 1);
  aos_free(buffer);
}
...
void dda_p2p_request(int dst_id, const char *cmd, dda_p2p_cb cb,
                     void *cb_data) {
  ...
  send_data_ext(&agent_info, TYPE_P2P, CMD_P2P_REQ, req->seqno, dst_id, cmd,
                strlen(cmd) + 1);
}
...
void dda_mesh_send_data(int type, int cmd, uint64_t cmd_pri, int dst_id,
...
uint64_t dda_mesh_get_agent_id(void) { return agent_info.agent_id; }
