...
static void netmgr_connect_fail_event(hal_wifi_module_t *m, int err,
                                      void *arg) {}
...
static void update_seed_history(seed_history_t *history,
...
static void netmgr_stat_chg_event(hal_wifi_module_t *m, hal_wifi_event_t stat,
  ...
  case NOTIFY_STATION_UP:
    ...
    if (0 == rand_flag) { // Do the rand operation only once
      ...
      dump_seed_history(&seed_history);
      ret = aos_kv_set(TCP_LOCAL_PORT_SEED, &seed_history, sizeof(seed_history),
                       1);
#endif /* LOCAL_PORT_ENHANCED_RAND */
      rand_flag = 1;
    }
...
static void get_bssid(uint8_t *to_fill, int size) { memset(to_fill, 0, size); }
...
static void netmgr_events_executor(input_event_t *eventinfo, void *priv_data) {
  ...
  case CODE_WIFI_CMD_RECONNECT:
    g_netmgr_cxt.disconnected_times = 0;
    g_netmgr_cxt.ip_available = false;
    LOGD("netmgr", "reconnect wifi - %s, %s", g_netmgr_cxt.ap_config.ssid,
...
void wifi_get_ip(char ips[16]) { format_ip(g_netmgr_cxt.ipv4_owned, ips); }

void netmgr_register_wifi_scan_result_callback(
    netmgr_wifi_scan_result_cb_t cb) {
  g_netmgr_cxt.cb = cb;
...
void netmgr_clear_ap_config(void) { clear_wifi_ssid(); }
...
void netmgr_deinit(void) { memset(&g_netmgr_cxt, 0, sizeof(g_netmgr_cxt)); }
...
bool netmgr_get_ip_state() { return g_netmgr_cxt.ip_available; }
